...
import { Delete as DeleteIcon, Edit as EditIcon, Group } from '@mui/icons-material';
...
import { getTeamName, isTeamLeader } from '../../utils/helpers.js';
...
export default function TeamManagement({ onUpdate }) {
  ...
  const fetchData = async () => {
    ...
    const teams = responses[0].data?.success ? responses[0].data.data : responses[0].data || [];
    const users = responses[1]?.data?.success ? responses[1].data.data : responses[1]?.data || [];
  ...
  const filteredTeams = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      teams = teams.filter(team => team.name.toLowerCase().includes(searchTerm.toLowerCase()));
    }

    if (user.role === 'manager') {
      teams = teams.filter(team => {
        const isLeader = Number(team.leaderId) === Number(user.id);
        const isMember = team.members?.some(memberId => Number(memberId) === Number(user.id));
        return isLeader || isMember;
      });
    }

    return teams;
  }, [data, searchTerm, user.id, user.role]);

  if (loading) return <LoadingSpinner size={40} mt={4} />;

  if (error) {
    return (
      <Typography sx={{ color: '#d45454', p: 2 }}>
    ...
    return (
      ...
      >
        <Typography sx={{ color: '#888888' }}>
          You don't have access to Team Management. This page is for administrators and managers
          only.
        </Typography>
      </Paper>
    );
    ...
    <Box>
      ...
      >
        ...
        <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
          <SearchBar value={searchTerm} onChange={setSearchTerm} placeholder="Search teams..." />
          {canManage ? (
            <Button
              variant="contained"
              size="medium"
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ backgroundColor: '#0d0d0d' }}>
                {['Team Name', 'Leader', 'Members', ...(canManage ? ['Actions'] : [])].map(
                  label => (
                    <TableCell
                      key={label}
                      sx={{
              ...
              {filteredTeams.map(team => {
                ...
                const members =
                  team.members
                    ...
                    .filter(Boolean) || [];
                const isLeader = isTeamLeader(team, user.id);
                  ...
                  >
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        ...
                        <Box>
                          ...
                          {!isLeader && Number(team.leaderId) === Number(user.id) && (
                            <Chip
                              size="small"
                              label="You are Leader"
                              sx={{
                                mt: 0.5,
                                backgroundColor: 'rgba(240,160,48,0.12)',
                                color: '#f0a030',
                        ...
                        {members.map(m => (
                          <Chip
                            ...
                            sx={{
                              ...
                              color: Number(m.id) === Number(team.leaderId) ? '#f0a030' : '#888888',
                              border:
                                Number(m.id) === Number(team.leaderId)
                                  ? '1px solid rgba(240,160,48,0.25)'
                                  : '1px solid #2a2a2a',
                    ...
                    {canManage && (
                      <TableCell align="right">
                        <IconButton
                          size="small"
                          onClick={() => setDialog({ open: true, editing: team })}
                          sx={{
                            color: '#888888',
                            '&:hover': {
                              color: '#6c63ff',
